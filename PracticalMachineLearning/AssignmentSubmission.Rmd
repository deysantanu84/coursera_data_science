...
```{r feature_plot, fig.width=10, fig.height=8, dpi=72}
...
```{r corrplot, fig.width=10, fig.height=10, dpi=72}
...
```{r tsne_plot, fig.width=10, fig.height=8, dpi=72}
...
Best cross-validation's minimum error rate `test_merror_mean` is around 0.006 (0.6%), happened at 151st iteration.
...
The average accuracy is `99.4%`, with error rate is `0.6%`. So, expected error rate of less than `1%` is fulfilled.
...
```{r xgb_plot, fig.width=12, fig.height=16, dpi=72}
